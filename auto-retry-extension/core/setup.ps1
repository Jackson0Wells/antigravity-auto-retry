...
function Update-UserShortcuts {
    ...
    $targetScript = if ($RunHidden) { "launch_ide_and_retry_hidden.vbs" } else { "launch_ide_and_retry_visible.vbs" }
    $launcherPath = Join-Path $scriptDir $targetScript
    ...
    foreach ($root in $shortcutRoots) {
        ...
        foreach ($shortcut in $shortcuts) {
            try {
                $target = Resolve-ShortcutTarget -ShortcutPath $shortcut.FullName
                if ($target -and (($target -match "launch_ide_and_retry") -or ((Split-Path -Leaf $target) -like "*Antigravity*.exe"))) {
                    $sc = $shell.CreateShortcut($shortcut.FullName)
...
$saveAction = {
    ...
    Update-UserShortcuts -IdePath $idePath -RunHidden $runHiddenCheckbox.Checked
    $statusLabel.Text = "Saved config.json and updated shortcuts."
    return $true
}
